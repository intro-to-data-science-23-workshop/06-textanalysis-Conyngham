...
inaugural_speech_tokens  <-  tokens_remove(inaugural_speech_tokens, c('the','and', 'that','to', 'can', 'must', 'of', 'every', 'words', 'let', 'end'))
...
**Step 2**
We'll compare it with another speech, this time with "An Honest Government - A Hopeful Future" which Obama hold at the University of Nairobi on about the topic: Our Past, Our Future & Vision for America (August 28, 2006). Updates.
...
With Selector Gadget we identify the structure in the html containing the text. Copy it direct from the bar, do not click on the xpath function. 
```{r}
honest_gov_speech_container <- obama_honest_gov |>  html_nodes("br+ table font+ font")
...
honest_gov_speech_tokens  <-  tokens_remove(honest_gov_speech_tokens, c('the','and', 'that','to', 'can', 'must', 'of', 'every', 'words', 'let', 'end'))
...
Let's see the wordcloud from this speech
```{r}
textplot_wordcloud(honest_gov_speech_dfm, max_words = 50)


```


With both discourses we can check the

Distance and similarity
```{r}
as.data.frame(textstat_dist(honest_gov_speech_dfm)) %>% #Here we run the function, with the default, euclidean distance measurement.
  
  filter(document2 == yourpresentationname) %>% arrange(euclidean) %>% select (c(1,3)) %>% gt()
```
