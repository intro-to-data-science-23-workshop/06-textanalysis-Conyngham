...
#TO DO: Explain how tokenization can be skipped when applying dfm() right away --> Luca
...
The readtext package provides an easy way to read text data into R, from almost any
input format. Compatible input formats include .txt, .csv, .tab, .json, .pdf, .doc and
.docx files
...
It is also possible to explore the corpus:
```{r}summary(corpus_name)```

Extract or add document-level variables: 
```{r}docvars()```

Other relevant corpus functions include:
#Discuss: Do we want to taylor the example functions to our use case or would that take too much time now?
...
#TO DO: Add short explanations what these functions do --> Luca
#TO DO: Add short explanation based on our use case how we clean + tokenize the data -->
Aranxa
